...
if(isset($_GET) && !empty($_GET))  {
    $CartonId = $_GET['CTNId']; 
    $CustomerData ="SELECT carton.CustId1,carton.CTNOrderDate,carton.CTNFinishDate,ppcustomer.CustName,ppcustomer.CustMobile,ppcustomer.CustAddress 
...
<div class="card  m-3">
    <div class="card-body d-flex justify-content-between">
        <a class= "btn btn-outline-primary  " href="CustomerProfile.php?id=<?=$CustomerId?>" >
        ...
        <a  onclick = "Print()" class="btn btn-outline-primary  my-1"  title = "Click to Print Customer List ">
            ...
            <a   class="btn btn-outline-danger   " id = "agrement-button"  onclick = "RemoveAgreement('Agreement');alert('Agreement Removed From Print');"  >
...
<div class="card m-3"> <!-- card start div -->
    <div class="card-body"> <!-- card body start div -->
        ...
        <?php 
            $total=0; 
            $Query='SELECT CTNId,ProductName,  CONCAT( FORMAT(CTNLength / 10 ,1 ) , " x " , FORMAT ( CTNWidth / 10 , 1 ), " x ", FORMAT(CTNHeight/ 10,1) ) AS Size  ,CTNUnit,CTNColor,CTNPaper,CTNQTY,CTNPrice,CTNTotalPrice,MarketingNote,CTNOrderDate,CTNFinishDate,
            CFluteType,CTNType,CTNUnit,CTNPolimarPrice,FinalTotal,CTNDiePrice,CtnCurrency,employeet.Ename , `Ctnp1`, `Ctnp2`, `Ctnp3`, `Ctnp4`, `Ctnp5`, `Ctnp6`, `Ctnp7` , Tax 
            FROM carton LEFT JOIN employeet ON employeet.EId=carton.EmpId WHERE CTNId = ?';
        ...
        <div class="table-responsive  mt-5"><!-- table start div -->
            ...
            $i = 1 ; 
                            echo "<tr>";
                                echo "<td>".$i."</td>";
                                echo "<td>".$Row['CTNId']."</td>";
                                echo "<td>". $Row['ProductName'] .' ( ' . $Row['Size'] . " )  "."</td>";
                                ?>  
                                <td> 
                                <?php
                                ...
                                { 
                                  if(empty($Row['Ctnp'.$index])) continue; 
                                  $arr[] = $Row['Ctnp'.$index];   
                                } 
                                ...
                                <?php  
                                echo "<td>".$Row['CTNColor']."</td>";
                                echo "<td>".$Row['CTNUnit']."</td>";
                                echo "<td>".number_format($Row['CTNQTY'])."</td>";
                                echo "<td>".number_format($Row['CTNPrice'],2)."</td>";
                                echo "<td>".number_format($Row['CTNPolimarPrice'])."</td>";
                                echo "<td>".number_format($Row['CTNDiePrice'])."</td>";
                                echo "<td>".number_format($Row['FinalTotal'])."</td>";
                                if ($Row['Tax'] != 0) {
                                    echo "<td>".$Row['Tax']." % </td>";
                                    echo "<td>";
                                    echo (($Row['FinalTotal'] * $Row['Tax']) / 100)  ;
                                    echo "</td>";
                                    
                                }

                                echo "</tr>";
                                $total=$total+$Row['FinalTotal'];
                                $i++;                    
            ...
            ?>
                    <tr>
                        ...
                        <td colspan='5' class="text-center fw-bold"> <?php echo number_format($total)." <span class='badge bg-warning'> ". $Row['CtnCurrency']; ?> </span> </td>
                    </tr>
                </tbody>
            </table>
        </div><!-- table end div -->
...
<div class="card m-3"> <!-- card start div -->
    <div class="card-body fs-4 pe-5"  style="direction: rtl; unicode-bidi: bidi-override; "> <!-- card body start div -->
        <p style = "margin:0px; padding:0px; " ><?php echo $Row['MarketingNote']; ?></p>
    </div> <!-- card body end div -->
</div> <!-- card end div -->
    ...
    <div class="card-body"> <!-- card body start div -->
        ...
        <div class="row"> <!-- start of row div -->
            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 text-center mb-5 mt-3">
                <span class = "fw-bold">Prepared By: </span><?php echo $Row['Ename']; ?> 
            </div>
...
<div class="card m-3"><!-- card start div -->
    ...
    <?php
            ...
            $DataRows = $Controller->QueryData($Query2, [$CustomerId]);
...
<div class="modal fade" style = "font-family: Roboto,sans-serif;" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      ...
      <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) .'?'. 'CTNId=' . $CartonId ?>" method = "post" >
...
<script type="text/javascript">
    ...
    function RemoveAgreement(id){
        // let a = document.getElementById('Print_Area_Agreement').style.display = 'none'; 
...
<div id="print_area" class = "m-5" style = "display:none;  width:21cm " >
    ...
    <div class="m-1" style = "width:21cm; padding:1cm ; " > <!-- card start div -->
        ...
        <div  style = "margin-top:1cm; "  > 
            <h4 style = "width: 100%;   border-bottom: 8px solid  #1b4fd1;   line-height: 0.1em; " >
    ...
    <?php 
        $Query='SELECT CTNId,ProductName,  CONCAT( FORMAT(CTNLength / 10 ,1 ) , " x " , FORMAT ( CTNWidth / 10 , 1 ), " x ", FORMAT(CTNHeight/ 10,1) ) AS Size  ,CTNUnit,CTNColor,CTNPaper,CTNQTY,CTNPrice,CTNTotalPrice,MarketingNote,CTNOrderDate,CTNFinishDate,
        CFluteType,CTNUnit,CTNType,CTNPolimarPrice,FinalTotal,CTNDiePrice,CtnCurrency,employeet.Ename , `Ctnp1`, `Ctnp2`, `Ctnp3`, `Ctnp4`, `Ctnp5`, `Ctnp6`, `Ctnp7` , Tax 
        FROM carton LEFT JOIN employeet ON employeet.EId=carton.EmpId WHERE CTNId = ? ORDER BY carton.`CTNId` DESC  ';
        $DataRows  = $Controller->QueryData($Query, [$CartonId]);
        $Row= $DataRows ->fetch_assoc(); 
     ?>
     <div style = "  margin-top:1cm; " ><!-- table start div -->
            ...
            <thead>
                <tr style = "border:1px solid; font-size:12px; font-weight:bold;  height:25px;">
                    ...
                    <?php if ($Row['Tax'] != 0) { ?>
                        ...
                        <th scope="col" style = "border:1px solid;" >Tax Amount</th>
                        <?php } ?>
                </tr>
            </thead>
            <tbody>
        <?php
            $total=0;  $i = 1 ; 
                            ...
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:center; ' >".$Row['CTNId']."</td>";
                            echo "<td> ".$Row['ProductName'] .' ( ' . $Row['Size'] . ' )'." </td>";
                            
                            echo"<td style = 'border:1px solid ;padding:3px; ' >"; 
                            ...
                            echo "<td style = 'border:1px solid ;padding:3px;text-align:center;' >".$Row['CTNColor']."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px;text-align:center;' >".$Row['CTNUnit']."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".number_format($Row['CTNQTY'])."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".number_format($Row['CTNPrice'],2)."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".$Row['CTNPolimarPrice']."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".$Row['CTNDiePrice']."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".number_format($Row['FinalTotal'])."</td>";
                            if ($Row['Tax'] != 0) {
                                echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".$Row['Tax']." % </td>";
                                echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >";
                                echo(($Row['FinalTotal'] * $Row['Tax']) / 100)  ;
                                echo "</td>";
                            }
                            echo "</tr>";
                ...
                <tr style = "height:25px;" >
                    ...
                    <td colspan='5' style = 'border:1px solid; text-align:right; padding:3px; font-weight:bold;' > <?php echo number_format($total)." - <span style = 'font-weight:bold; font-size:10px; ' >". $Row['CtnCurrency']  . '</span>'; ?>   </td>
                </tr>
    ...
    <div style="display: flex;  justify-content: space-between; padding-left:20px; padding-right:20px; margin-top:3cm;"> <!-- start of row div -->
            <div> <span style = "font-weight:bold;" >Prepared By: </span> <?php echo $Row['Ename']; ?> <br>
            <span style = "font-weight:bold;" >Issued Date: </span> <?php echo date('H:i:s d-m-Y') ?> </div>
...
<!-- /#0dcaf0!important;  -->
    ...
    <div  style = "  margin-top:2cm;"   >
        ...
        <h4 style = "width: 100%;   border-bottom: 8px solid  #1b4fd1;  line-height: 0.1em;  margin-top:0px;   " >
    ...
    <div style="margin-top:4cm; display:none; " id = "Print_Area_Agreement" > 
    <?php
