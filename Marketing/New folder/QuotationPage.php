    ...
    if(isset($_POST) && !empty($_POST))   
    {
        ...
        if( isset($_POST['design']) && !empty($_POST['design'])  )  {
            ...
            if(isset($_POST['CustId']) && !empty($_POST['CustId'])){
                    ...
                    $CustomerData ="SELECT DISTINCT  carton.CustId1,carton.CTNOrderDate,carton.CTNFinishDate,ppcustomer.CustName,ppcustomer.CustMobile,ppcustomer.CustAddress 
        ...
        <div class="table-responsive  mt-5"><!-- table start div -->
            ...
            <?php
                $total=0; $i=1;
                    ...
                    {
                        $Query='SELECT CTNId,ProductName, CONCAT( CTNWidth, " x ", CTNHeight , " x " , CTNLength ) AS Size ,CTNUnit,CTNColor,CTNPaper,CTNQTY,CTNPrice,CTNTotalPrice,MarketingNote,CTNOrderDate,CTNFinishDate, CFluteType,CTNType,CTNPolimarPrice,FinalTotal,CTNDiePrice,CtnCurrency,employeet.Ename , `Ctnp1`, `Ctnp2`, `Ctnp3`, `Ctnp4`, `Ctnp5`, `Ctnp6`, `Ctnp7` FROM carton INNER JOIN employeet ON carton.EmpId = employeet.EId WHERE CTNId = ?';
...
if ($DataRows->num_rows > 0) {
    ...
    echo " , $no F" .  " - ". $Rows['CTNType']. " Ply ". " (". $Rows['CFluteType'].")" ;
    ?>
                                </td>
                                <?php
                                echo "<td>".$Rows['CTNColor']."</td>";
    echo "<td>".$Rows['CTNUnit']."</td>";
    echo "<td>".$Rows['CTNQTY']."</td>";
    echo "<td>".$Rows['CTNPrice']."</td>";
    echo "<td>".$Rows['CTNPolimarPrice']."</td>";
    echo "<td>".$Rows['CTNDiePrice']."</td>";
    echo "<td>".$Rows['FinalTotal']."</td>";
    echo "</tr>";
    ...
    $i++;
            ...
            ?>
                    <tr>
                        ...
                        <td colspan='5' class="text-center fw-bold"> <?php  if(isset($Rows['CtnCurrency'])) {
    echo $total." <span class='badge bg-warning'> ". $Rows['CtnCurrency'];
}?> </span> </td>
                    </tr>
                </tbody>
            </table>
        ...
        <?php
                foreach ($_POST['design'] as $check1)
                {
                    ...
                    <div class = "p-3 mb-3" style = "border:3px solid black; border-radius:5px;  " >
                        <img class = "img-fluid" src="../Public/<?=$Rows['DesignImage']?>"> 
                    </div>
...
<div class="m-1" style = "width:21cm; padding:1cm ; " > <!-- card start div -->
    ...
    <div  style = "margin-top:1cm; "  >
        <h4 style = "width: 100%;   border-bottom: 8px solid #0dcaf0!important;  line-height: 0.1em; " >
     ...
     <div style = "  margin-top:1cm; " ><!-- table start div -->
        ...
        <?php
            $total=0; $i=1;
                foreach ($_POST['design'] as $check) 
                {
                    $Query='SELECT CTNId,ProductName,  CONCAT( CTNWidth, " x ", CTNHeight , " x " , CTNLength ) AS Size ,CTNUnit,CTNColor,CTNPaper,CTNQTY,CTNPrice,CTNTotalPrice,MarketingNote,CTNOrderDate,CTNFinishDate,
                    CFluteType,CTNType,CTNPolimarPrice,FinalTotal,CTNDiePrice,CtnCurrency,employeet.Ename  , `Ctnp1`, `Ctnp2`, `Ctnp3`, `Ctnp4`, `Ctnp5`, `Ctnp6`, `Ctnp7`   FROM carton INNER JOIN employeet 
                    ON employeet.EId=carton.EmpId WHERE CTNId = ? ORDER BY carton.`CTNId` DESC  ';
                    $DataRows  = $Controller->QueryData($Query, [$check]);
                    $Rows= $DataRows ->fetch_assoc();
                            ...
                            <td style = 'border:1px solid ;padding:3px; ' > 
                                <?php 
                                    ...
                                    if($Rows['Ctnp1']=='Flute')
                                    {$no=$no+1;}
                                    else{ echo $Rows['Ctnp1']; }
                                    if($Rows['Ctnp2']=='Flute')
                                        {$no=$no+1;} else{echo $Rows['Ctnp2'] ;}
                                    if($Rows['Ctnp3']=='Flute')
                                        {$no=$no+1;} else{echo $Rows['Ctnp3'];}
                                    if($Rows['Ctnp4']=='Flute')
                                        {$no=$no+1;}  else{echo $Rows['Ctnp4'];}
                                    if($Rows['Ctnp5']=='Flute')
                                        {$no=$no+1;} else{echo $Rows['Ctnp5'];}
                                    if($Rows['Ctnp6']=='Flute')
                                        {$no=$no+1;} else{echo $Rows['Ctnp6'];}
                                    if($Rows['Ctnp7']=='Flute')
                                        {$no=$no+1;} else{echo $Rows['Ctnp7'];}
                                    echo ", $no F" .  " - ". $Rows['CTNType']. " Ply ". " (". $Rows['CFluteType'].")" ;
                                ?>
                            </td>
                            <?php  
                            ...
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".$Rows['CTNQTY']."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".$Rows['CTNPrice']."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".$Rows['CTNPolimarPrice']."</td>";
                            echo "<td style = 'border:1px solid ;padding:3px; text-align:right;' >".$Rows['CTNDiePrice']."</td>";
                ...
                <tr style = "height:25px;" >
                    ...
                    <td colspan='5' style = 'border:1px solid; text-align:right; padding:3px; font-weight:bold;' > <?php echo $total." - <span style = 'font-weight:bold; font-size:10px; ' >". $Rows['CtnCurrency']  . '</span>'; ?>   </td>
                </tr>
    ...
    <div style="display: flex;  justify-content: space-between; padding-left:20px; padding-right:20px; margin-top:3cm;"> <!-- start of row div -->
            <div> <span style = "font-weight:bold;" >Prepared By: </span> <?php echo $Rows['Ename']; ?>  </div>
    ...
    <div  style = "margin-top:2cm;"   >
        ...
        <h4 style = "width: 100%;   border-bottom: 8px solid #0dcaf0!important;  line-height: 0.1em;  margin-top:0px;   " >
                ...
                foreach ($_POST['design'] as $check1)
                {
                    ...
                    if (!empty($Rows['DesignImage'])) {   ?>	
                        <div style = "padding:10px; margin-bottom:1cm;  border:2px solid black; border-radius:5px;  " >
                            <img style = "width:100%; " src="../Public/<?=$Rows['DesignImage']?>"> 
                        </div>
                    <?php
                    }
                }
    ...
    <div style="margin-top:2cm;" id = "Print_Area_Agreement" style = "" > 
    <?php
