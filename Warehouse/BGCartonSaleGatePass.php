...
if(isset($_GET['CTNId']) && !empty($_GET['CTNId'])) {
        ...
        $SQL=$Controller->QueryData('SELECT carton.CTNId,carton.JobNo,carton.ProductName,carton.CTNType,CONCAT(FORMAT(CTNLength/10,1),"x",FORMAT(CTNWidth/10,1),"x",FORMAT(CTNHeight/ 10,1)) AS Size ,
        ppcustomer.CustName,CtnDriverMobileNo,CtnDriverName,CtnCarName,CoutComment,CtnOutQty 
        ,CtnCarNo, cartonstockout.OutDateTime,cartonstockout.CtnoutId FROM cartonstockout INNER JOIN carton ON cartonstockout.CtnJobNo=carton.CTNId  
        INNER JOIN ppcustomer ON cartonstockout.CtnCustomerId=ppcustomer.CustId  WHERE  CtnoutId=?',[$CtnoutId]);
