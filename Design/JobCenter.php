...
<div class="card m-3 shadow ">
  <div class="card-body d-flex justify-content-between    ">
    ...
    <div>
    <div>  <span class = "fs-bold fs-4  ps-2 " >Design Job Center</span></div>
      <div  class = "fs-bold  ps-3" > <span class = 'badge bg-success' > <?php if(isset($_REQUEST['ListType'])){ echo $_REQUEST['ListType'];}else{ echo 'NewJob';} ?> </span> </div>
    </div>
    </div>
          ...
          $Count = 1; 
            while($Rows=$DataRows->fetch_assoc()) {    ?>
                ...
                ?>
                      ...
                      <td>
                          <a class="btn btn-outline-warning" data-bs-toggle="collapse" href="#colapse1<?=$COUNTER?>" role="button" aria-expanded="false"  > 
                            <svg width="15" height="15" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M20 10C20 11.104 19.104 12 18 12H2C0.896 12 0 11.104 0 10C0 8.896 0.896 8 2 8H18C19.104 8 20 8.896 20 10Z" fill="black"></path>
                            <path d="M20 2C20 3.104 19.104 4 18 4H2C0.896 4 0 3.104 0 2C0 0.896 0.896 0 2 0H18C19.104 0 20 0.896 20 2Z" fill="black"></path>
                            <path d="M20 18C20 19.104 19.104 20 18 20H2C0.896 20 0 19.104 0 18C0 16.896 0.896 16 2 16H18C19.104 16 20 16.896 20 18Z" fill="black"></path>
                            </svg> <span class="fw-bold">   </span>
                          </a>

                          <div class="collapse" style="position: absolute; z-index: 1000; width: 15%; left: 84%; margin-top: 15px;" id="colapse1<?=$COUNTER++?>">
                            <div class="card card-body border shadow">
                                <a href="DesignManage.php?CTNId=<?=$Rows['CTNId']?>&ListType=<?=$ListType?>" class="btn btn-outline-success btn-sm m-1 border-3 fw-bold">
                                Manage
                                </a> 
                               
                                <!-- <a href="DesignStateChanger.php?CTNId=<?=$Rows['CTNId']?>&JobNo=<?=$Rows['JobNo']?>&ListType=<?=$ListType?>" class="btn btn-outline-info btn-sm m-1 border-3 fw-bold"
                                    onclick="return confirm(`Did You check the Design of this Job Or Qoutation`);">
