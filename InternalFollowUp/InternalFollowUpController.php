...
class InternalFollowUpController
{
    public function InsertFollowUp($Request , $Controller ){
        ...
        $query = "INSERT INTO follow_up(  follow_result , deadline , comment , customer_id , CTNId)  VALUES ( ?,?,?,?,? )";
        $results = $Controller->QueryData($query , [  $follow_result  ,  $deadline ,$comment, $customer_id , $CTNId ]);
        
        if ($results ) {
            header('Location:FollowUp.php?msg=100&CustId='.$customer_id . '&CTNId='.$CTNId); 
        } else {
            header('Location:FollowUp.php?msg=101'.$customer_id . '&CTNId='. $CTNId ); 
        }
    }
